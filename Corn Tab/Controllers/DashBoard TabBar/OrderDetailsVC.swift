...
class OrderDetailsVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        itemCounts = addedItems.compactMap { Int($0["quantity"] ?? "0") }
        tableView.reloadData()
        calculateAndUpdateTotal()
        print(addedItems.description)
...
//        }
    ...
    @objc func plusBtnTapped(sender: UIButton) {
        ...
        if let indexPath = tableView.indexPathForRow(at: point) {
            if var quantity = Int(addedItems[indexPath.row]["quantity"] ?? "") {
                quantity += 1
                addedItems[indexPath.row]["quantity"] = String(quantity)
                calculateAndUpdateTotal()
    ...
    private func calculateAndUpdateTotal() {
        let subtotalPrice = addedItems.reduce(0.0) { result, item in
            let priceKey = item["itemINCV"] != nil ? "itemPrice" : "price"
            let quantityKey = item["itemINCV"] != nil ? "itemINCV" : "quantity"
        ...
        let taxAmount = subtotalPrice * taxPercentage
        let totalPrice = subtotalPrice + taxAmount
        let numberFormatter = NumberFormatter()
        numberFormatter.numberStyle = .decimal
        numberFormatter.minimumFractionDigits = 2
        subTotalLbl.text = "Rs: " + (numberFormatter.string(from: NSNumber(value: subtotalPrice)) ?? "")
        taxIncludedLbl.text = "Rs: " + (numberFormatter.string(from: NSNumber(value: taxAmount)) ?? "")
...
//        }
        ...
        if let item = item["quantity"], !item.isEmpty {
